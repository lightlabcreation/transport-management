...
describe('CreateGroupPage Wizard Flow', () => {
  it('renders Step 1 (Group Information) initially with fields and category dropdown', () => {
    ...
    expect(screen.getByRole('heading', { level: 1, name: /Create New Group/i })).toBeInTheDocument();
  ...
  it('runs through the entire wizard flow, configures settings, requires terms, and completes submission', async () => {
    ...
    await user.type(screen.getByLabelText(/Description \/ Purpose/i), 'Velo transportation details.');
    await user.click(screen.getByRole('button', { name: /Next/i }));
    ...
    // Step 3: Tracking
    expect(screen.getByRole('heading', { level: 2, name: /Tracking Settings/i })).toBeInTheDocument();
    ...
    // Step 4: Visibility
    expect(screen.getByRole('heading', { level: 2, name: /Visibility Policy/i })).toBeInTheDocument();
    ...
    expect(screen.getByRole('heading', { level: 2, name: /Roles & Permissions Configurator/i })).toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: /Next/i }));

    // Step 6: Review & Terms
    expect(screen.getByRole('heading', { level: 2, name: /Review Group Details/i })).toBeInTheDocument();
    ...
    expect(screen.getByText(/Background Updates:/i).parentElement?.textContent).toContain('Background Updates: Disabled');
    ...
    expect(screen.getByText(/You must accept the Terms and Conditions to proceed./i)).toBeInTheDocument();
    ...
    await waitFor(
      () => {
        expect(screen.getByRole('heading', { level: 2, name: /Group Registration Pending Approval/i })).toBeInTheDocument();
      },
      { timeout: 2000 }
    );

    // Verification of pending simulated statement
    expect(screen.getByText(/This is a frontend simulation. Real-world payment gateways/i)).toBeInTheDocument();
